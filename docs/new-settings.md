...
| **Inline Matrices** | on/off | Print the matrices and vectors of the substitution step on a single bracketed line. On (default) uses inline output; off uses a grid of aligned cells inside full-height brackets. Calculated results are always printed as a grid. |
